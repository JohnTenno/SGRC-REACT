...
export function TutoringProfessorAdminCard({
  professor,
  isEditing = false,
  isDeleting = false,
  onEdit,
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <TutoringProfessorAvatar fullName={professor.fullName} size="md" />
        <div className="min-w-0 flex-1">
          <h2 className="font-alverata text-lg font-semibold text-uach-purple-900">{professor.fullName}</h2>
          <p className="font-praxis mt-2 text-sm leading-relaxed text-uach-purple-900/75 line-clamp-3">
            {professor.bio || 'Sin perfil breve.'}
          </p>
          <p className="font-praxis mt-3 text-xs text-uach-purple-900/50">
            No. empleado: {professor.employeeNumber}
          </p>
        </div>
      </div>
