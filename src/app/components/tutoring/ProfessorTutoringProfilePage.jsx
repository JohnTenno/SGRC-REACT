...
import { ReservationTimePicker } from '@/app/components/cubicles/ReservationTimePicker'
import { TutoringDateCarousel } from '@/app/components/tutoring/TutoringDateCarousel'
import { TutoringProfessorHero } from '@/app/components/tutoring/TutoringProfessorHero'
import {
  getTutoringProfessorByEmployeeNumber,
  professorTeachesSubject,
} from '@/app/components/tutoring/tutoringProfessors'
import { getTutoringSubjectById } from '@/app/services/tutoring/subjects.service'
import { getAvailableTutoringSlots } from '@/app/components/tutoring/tutoringAvailability'
import {
  getTutoringEndTimeOptions,
  submitTutoringRequest,
  validateTutoringRequest,
} from '@/app/services/tutoring/requests.service'
import {
  addDaysToIso,
  formatTutoringDateLabel,
  todayIso,
} from '@/app/components/tutoring/tutoringProfessorUtils'
...
export function ProfessorTutoringProfilePage() {
  ...
  const [subjectRecord, setSubjectRecord] = useState(undefined)
  const [professor, setProfessor] = useState(undefined)
  const [teachesSubject, setTeachesSubject] = useState(undefined)

  useEffect(() => {
    if (!Number.isFinite(subjectId)) return
    getTutoringSubjectById(subjectId)
      .then(setSubjectRecord)
      .catch(() => setSubjectRecord(null))
  }, [subjectId])
  ...
  useEffect(() => {
    if (!professor || !subjectRecord) return
    professorTeachesSubject(professor.employeeNumber, subjectRecord.id)
      .then(setTeachesSubject)
      .catch(() => setTeachesSubject(false))
  }, [professor, subjectRecord])
  ...
  const hasSlotsOnDate = useCallback(
    (isoDate) => (professor ? getAvailableTutoringSlots(professor, isoDate).length > 0 : false),
    [professor],
  )
  ...
  const formReady = Boolean(
    employeeNumber && subject && reservationDate && startTime && endTime && topic.trim(),
  )

  if (subjectRecord === undefined || professor === undefined || teachesSubject === undefined) {
    return null
  }

  if (!subjectRecord || !professor || !teachesSubject) {
