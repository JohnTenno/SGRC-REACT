 ...
 * @property {import('@/equipment/home/mockEquipment').MockEquipment & { quantity: number }[]} items
 */
...
function persistRequestForAdmin(request) {
...
function simulateCreateEquipmentRequest(selections) {
  return new Promise((resolve) => {
    setTimeout(() => {
      const request = normalizeEquipmentRentalRequest(
        {
      ...
      persistRequestForAdmin(request)
      resolve(request)
    }, 900)
  })
}
...
export async function createEquipmentRentalRequest(selections) {
  ...
  try {
    const request = normalizeEquipmentRentalRequest(data, selections)
    persistRequestForAdmin(request)
    return request
  } catch (error) {
    throw {
